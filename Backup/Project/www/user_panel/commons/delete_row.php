	...
	//{
		...
		if($id){
			...
			if($table_name == "users")
			{
				...
				$data_delete_photo = $db->fetchQueryPrepared($sql_delete_photo, array(':id' => $id));
			...
			if($table_name == "project_versions")
			{
				...
				$data_delete_file = $db->fetchQueryPrepared($sql_delete_file, array(':id' => $id));
			...
			$data_delete = $db->queryPrepared($sql_delete, array(':id' => $id))->rowCount();
			...
			if($data_delete == 1)
			{
				if(!empty($post_vars['join_entity']))
				{
					...
					$data_join_delete = $db->queryPrepared($sql_join_delete, array(':id' => $id))->rowCount();
				}
				$flag = true;
			}
